...
namespace MyRecipes.Transverse.Exception
{
    public class FoodTypeAlreadyExistException : ExceptionBase
    {
        ...
        public FoodTypeAlreadyExistException(ILogger<object> logger, string error, string message) : base(message) => logger.LogError(this, this.Message);

        public FoodTypeAlreadyExistException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public FoodTypeAlreadyExistException(ILogger<object> logger, string method, string sourceFilePath, string message) :
