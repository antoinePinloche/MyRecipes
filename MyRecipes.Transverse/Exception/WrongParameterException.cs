...
namespace MyRecipes.Transverse.Exception
{
    public class WrongParameterException : ExceptionBase
    {
        ...
        public WrongParameterException(ILogger<object> logger, string error, string message) : base(message) => logger.LogError(this, this.Message);

        public WrongParameterException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public WrongParameterException(ILogger<object> logger, string method, string sourceFilePath, string message) :
