...
namespace MyRecipes.Transverse.Exception
{
    public class RecipeAlreadyExistException : ExceptionBase
    {
        ...
        public RecipeAlreadyExistException(ILogger<object> logger, string error, string message) : base(message) => logger.LogError(this, this.Message);

        public RecipeAlreadyExistException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public RecipeAlreadyExistException(ILogger<object> logger, string method, string sourceFilePath, string message) :
