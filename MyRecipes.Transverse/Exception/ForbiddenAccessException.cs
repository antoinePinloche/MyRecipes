...
namespace MyRecipes.Transverse.Exception
{
    public class ForbiddenAccessException : ExceptionBase
    {
        ...
        public ForbiddenAccessException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public ForbiddenAccessException(ILogger<object> logger, string method, string sourceFilePath, string message) :
