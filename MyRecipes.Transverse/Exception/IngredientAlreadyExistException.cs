...
namespace MyRecipes.Transverse.Exception
{
    public class IngredientAlreadyExistException : ExceptionBase
    {
        ...
        public IngredientAlreadyExistException(ILogger<object> logger, string error, string message) : base(message) => logger.LogError(this, this.Message);

        public IngredientAlreadyExistException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public IngredientAlreadyExistException(ILogger<object> logger, string method, string sourceFilePath, string message) :
