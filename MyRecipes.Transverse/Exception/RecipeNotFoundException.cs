...
namespace MyRecipes.Transverse.Exception
{
    public class RecipeNotFoundException : ExceptionBase
    {
        ...
        public RecipeNotFoundException(ILogger<object> logger, string error, string message) : base(message) => logger.LogError(this, this.Message);

        public RecipeNotFoundException(string method, string sourceFilePath, string message) :
            base($"{Path.GetFileNameWithoutExtension(sourceFilePath)}.{method} : {message}")
        { }

        public RecipeNotFoundException(ILogger<object> logger, string method, string sourceFilePath, string message) :
